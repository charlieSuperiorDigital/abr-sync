...
interface WorkfilesContextType {
  workfiles: WorkfileApiResponse[]
  upcomingWorkfiles: WorkfileApiResponse[]
  inProgressWorkfiles: WorkfileApiResponse[]
  qualityControlWorkfiles: WorkfileApiResponse[]
  readyForPickupWorkfiles: WorkfileApiResponse[]
  subletsWorkfiles: WorkfileApiResponse[]
  laborWorkfiles: WorkfileApiResponse[]
  reportsWorkfiles: WorkfileApiResponse[]
  archiveWorkfiles: WorkfileApiResponse[]
  workfilesQuantity: {
    upcoming: number
    inProgress: number
...
export const WorkfilesProvider = ({ children }: WorkfilesProviderProps) => {
  ...
  }), [workfiles, upcoming, inProgress, qualityControl, readyForPickup, sublets, labor, reports, archive])
  ...
  useEffect(() => {
    if (session?.user.tenantId) {
      console.log(`Tenant ID from session for workfiles: ${session.user.tenantId}`)
    }
    
    if (workfiles && workfiles.length > 0) {
      console.log(`Loaded ${workfiles.length} workfiles for tenant ${session?.user.tenantId}`)
  ...
  return (
    <WorkfilesContext.Provider value={{ 
      workfiles: workfiles || [],
