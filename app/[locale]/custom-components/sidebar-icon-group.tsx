...
export default function SideBarIconGroup({ icons, link, label, expanded = false }: SideBarIconGroupProps) {
    const totalNotifications = icons.reduce((sum, icon) => sum + icon.newNotificationsQuantity, 0);
                    ...
                    <div className="flex flex-col space-y-2">
                        {icons.map((icon, index) => (
                            <Link
                                key={index}
                                href={link}
                                className="flex items-center py-1 cursor-pointer hover:bg-white hover:text-black rounded-lg px-2 transition-colors"
                            >
                                <SideBarIcon {...icon} />
                    ...
                    <div className="flex items-center">
                        ...
                        <div className="ml-4 font-medium text-base text-white group-hover:text-black whitespace-nowrap overflow-hidden">
                            ...
                            {totalNotifications > 0 && (
                                <span className="ml-2 font-bold text-red-500">{totalNotifications}</span>
                            )}
                        </div>
                    </div>
                </Link>
